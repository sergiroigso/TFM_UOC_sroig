...
```{r}
DimPlot(spl_seurat,group.by = "cluster_annotation_string", label = F, repel = TRUE, split.by = "orig.ident")
```
...
```{r}
DimPlot(spl_seurat,group.by = "SingleR_clean", label = F, repel = TRUE, split.by = "orig.ident")
```
...
```{r}
#"Trp53", "Cdkn1a", "Brca1", "Brca2", "H2ax", "Chek1", "Chek2", "Rad51"
DotPlot(
  spl_seurat,
  features = c("Trp53", "Brca1", "H2ax", "Chek2", "Rad51"),
  group.by = "SingleR_clean",
  split.by = "orig.ident"
) + 
  RotatedAxis() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
plots <- SplitObject(spl_seurat, split.by = "orig.ident")
...
# Obtener rangos comunes
max_color <- max(p_wt$data$avg.exp.scaled, p_ko$data$avg.exp.scaled)
min_color <- min(p_wt$data$avg.exp.scaled, p_ko$data$avg.exp.scaled)

max_size <- max(p_wt$data$pct.exp, p_ko$data$pct.exp)
min_size <- min(p_wt$data$pct.exp, p_ko$data$pct.exp)

# Reaplicar los límites manualmente
p_wt <- p_wt +
  ...
  scale_size(range = c(0, 8), limits = c(min_size, max_size))
...
# Reducir tamaño de los dots
p_wt <- p_wt + scale_size(range = c(0, 4))
p_ko <- p_ko + scale_size(range = c(0, 4))

(p_wt | p_ko) +
  plot_layout(guides = "collect") &
  theme(
    legend.position = "right",
    axis.text.y = element_text(size = 8)   # texto más pequeño
  )
...
bm_seurat$orig.ident <- factor(bm_seurat$orig.ident, levels = c("bm_WT", "bm_KO"))
...
```{r}
DimPlot(bm_seurat,group.by = "SingleR_clean", label = F, repel = TRUE, split.by = "orig.ident")
```
...
```{r}
plots <- SplitObject(bm_seurat, split.by = "orig.ident")
